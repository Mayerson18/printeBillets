...
namespace WindowsFormsApp1
{
    static class Program
    {
        ...
        static public void CreateLListener()
        {
            ...
            server.Prefixes.Add("http://localhost:8090/");//Abre el puerto
            server.Start();
            while (true)
            {
        ...
        public static void imprimirTest(string id, ref PrintPageEventArgs e, string zone, string chair, string table, string net_price, string iva, string total, string date, Font f, SolidBrush s, float x, ref float y, float w, float h, StringFormat sf)
        {
            //350 , 66
            ...
            float aux_y = 10;
            y = 0;
            Font regular4 = new Font("Verdana", 5, FontStyle.Regular);
            Font bold = new Font("Verdana", 5, FontStyle.Bold);
            System.Drawing.Image img = System.Drawing.Image.FromFile("TICKET.png");
            e.Graphics.DrawImage(img, new Rectangle(0, (int)Math.Ceiling(y), img.Size.Width, img.Size.Height));
            x += 250-aux_x;
            y += 65;
            e.Graphics.DrawString("0000" + id, f, s, new RectangleF(x - x_id, y - y_id, w, h), sf);
            e.Graphics.DrawString(zone, f, s, new RectangleF(x, y, w, h), sf);
            y += 30;

            e.Graphics.DrawString(table, f, s, new RectangleF(x, y, w, h), sf);
            y += 30;
            e.Graphics.DrawString(chair, f, s, new RectangleF(x, y, w, h), sf);
            y += 30;
            string aux_price2 = "PRECIO BASE:  " + net_price + " BS";
            string aux_price = "BASE:  " + net_price + " BS";
            string aux_iva = "IVA:  " + iva + " BS";
            string aux_total = "TOTAL:  " + total + " BS";
            float x_aux = -80-aux_x;
            float y_aux = 87;

            /*MEDIOOOOOOOOOOOOOOO */

            e.Graphics.DrawString(aux_price2, regular4, s, new RectangleF(x, y, w, h), sf);
            y += e.Graphics.MeasureString(aux_price2, regular4).Height;
            e.Graphics.DrawString(aux_iva, regular4, s, new RectangleF(x, y, w, h), sf);
            y += e.Graphics.MeasureString(aux_iva, regular4).Height;
            e.Graphics.DrawString(aux_total, regular4, s, new RectangleF(x, y, w, h), sf);
            y += e.Graphics.MeasureString(aux_total, regular4).Height;

            /*IZQUIERDA y DERECHA EVENTO,ZONA, SILLA*/
            e.Graphics.DrawString(zone, f, s, new RectangleF(x_aux, y_aux - 7, w, h), sf);
            e.Graphics.DrawString(zone, f, s, new RectangleF(374-aux_x, y_aux- 7, w, h), sf);
            y_aux += 30;
            e.Graphics.DrawString(table, f, s, new RectangleF(x_aux - 20, y_aux, w, h), sf);
            e.Graphics.DrawString(chair, f, s, new RectangleF(x_aux + 15, y_aux, w, h), sf);
            e.Graphics.DrawString(table, f, s, new RectangleF(374-aux_x - 20, y_aux, w, h), sf);
            e.Graphics.DrawString(chair, f, s, new RectangleF(374-aux_x + 20, y_aux, w, h), sf);


            /*IZQUIERDA IZQUIERDISTA*/
            y_aux = 140;
            e.Graphics.DrawString("29 Dic 2017", bold, s, new RectangleF(x_aux, y_aux, w, h), sf);
            y_aux += e.Graphics.MeasureString(date, bold).Height;

            e.Graphics.DrawString(aux_price, regular4, s, new RectangleF(x_aux, y_aux, w, h), sf);
            y_aux += e.Graphics.MeasureString(aux_price, regular4).Height;
            e.Graphics.DrawString(aux_iva, regular4, s, new RectangleF(x_aux, y_aux, w, h), sf);
            y_aux += e.Graphics.MeasureString(aux_iva, regular4).Height;
            e.Graphics.DrawString(aux_total, regular4, s, new RectangleF(x_aux, y_aux, w, h), sf);
            y_aux += e.Graphics.MeasureString(aux_total, regular4).Height;
